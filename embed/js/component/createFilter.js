...
function createFilter(markers, map, splitMarkerZoom) {
  ...
  createBarPrice(barGraph, markers, maxPrice);
  ...
  priceRange.append(lower, upper, lowerLbl, upperLbl, slideTrackwrap, barGraph)
...
function createBarPrice(barGraph, markers, maxPrice) {
  const barElement = document.querySelector(".bar-price");
  const totalBar = 40
  const priceEachBar = maxPrice / totalBar
  let maxCount = 0
  let dataItemCount = []
  for (let i = 0; i < totalBar; i++) {
    const barItemPrice = (i + 1) * priceEachBar;
    const minBarPrice = barItemPrice - priceEachBar;
    const propertyBarCount = markers.filter(item => item.price > minBarPrice && item.price <= barItemPrice)
    
    dataItemCount.push(propertyBarCount.length)
    if (propertyBarCount.length > maxCount) {
      maxCount = propertyBarCount.length
    }
  }
  if (!barElement) {
    addBarItem(barGraph, dataItemCount, maxCount)
  } else {
    replaceBarItem(barElement, dataItemCount, maxCount)
  }
}

function addBarItem(barGraph, dataItemCount, maxCount) {
  dataItemCount.map(item => {
    const barItem = document.createElement("li");
    const heightStyle = (item / maxCount) * 100;
    const widthStyle = 100 / dataItemCount.length;
    barItem.setAttribute("class", "bar-item")
    barItem.style.height = heightStyle + "%";
    barItem.style.width = widthStyle + "%";
    barItem.setAttribute("count", item)
    barGraph.appendChild(barItem);
  })
}

function replaceBarItem(barElement, dataItemCount, maxCount) {
  const barGraphNew = document.createElement("div");
  barGraphNew.setAttribute("class", "bar-price")
  dataItemCount.map(item => {
    const barItem = document.createElement("li");
    const heightStyle = (item / maxCount) * 100;
    const widthStyle = 100 / dataItemCount.length;
    ...
    barItem.style.width = widthStyle + "%";
    barItem.setAttribute("count", item)
    barGraphNew.appendChild(barItem);
  })
  barElement.parentNode.replaceChild(barGraphNew, barElement);
}
...
export { createFilter, createBarPrice, neighborhoodFilter }